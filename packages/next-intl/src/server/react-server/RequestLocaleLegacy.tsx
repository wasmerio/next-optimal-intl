...
function getLocaleFromHeaderImpl() {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        'Usage of next-optimal-intl APIs in Server Components currently opts into dynamic rendering. This limitation will eventually be lifted, but as a stopgap solution, you can use the `setRequestLocale` API to enable static rendering, see https://next-optimal-intl.dev/docs/getting-started/app-router/with-i18n-routing#static-rendering',
        {cause: error}
      );
    } else {
      throw error;
  ...
  if (!locale) {
    if (process.env.NODE_ENV !== 'production') {
      console.error(
        `\nUnable to find \`next-optimal-intl\` locale because the middleware didn't run on this request. See https://next-optimal-intl.dev/docs/routing/middleware#unable-to-find-locale. The \`notFound()\` function will be called as a result.\n`
      );
    }
    notFound();
  }
