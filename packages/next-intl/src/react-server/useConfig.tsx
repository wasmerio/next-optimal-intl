...
function useHook<Value>(hookName: string, promise: Promise<Value>) {
  try {
    ...
    if (
      ...
      throw new Error(
        `\`${hookName}\` is not callable within an async component. Please refer to https://next-optimal-intl.dev/docs/environments/server-client-components#async-components`,
        {cause: error}
      );
    } else {
      throw error;
